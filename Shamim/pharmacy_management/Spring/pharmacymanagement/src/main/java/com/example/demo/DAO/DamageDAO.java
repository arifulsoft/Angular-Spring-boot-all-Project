...
//			System.out.println("Stock Id "+stockid);
	...
	public void updateDamageStatus(List<Damage> u) {
		try {
			con = DriverManager.getConnection(url, user, pass);
			
			for (int i = 0; i < u.size(); i++) {
				ps = con.prepareStatement("update damage set status=? where damageId=?");
				ps.setString(1, u.get(i).getStatus());
				ps.setInt(2, u.get(i).getDamageId());
