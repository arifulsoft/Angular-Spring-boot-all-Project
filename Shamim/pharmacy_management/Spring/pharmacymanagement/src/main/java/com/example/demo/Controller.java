...
import com.example.demo.DAO.ExpiredDAO;
...
//	save Supplier 
	...
	// Save Damage
		@PutMapping("/update-damage")
		public void updatedamagestatus(@RequestBody List<Damage> d) {
			DamageDAO dao = new DamageDAO();
			System.out.println("Purchase => " + d);
			dao.updateDamageStatus(d);
		}
...
//			System.out.println("Purchase => " + d);
	...
	//  save and get Expired list 
			@GetMapping("/get-expired")
			public List<Expired> getExpiredList () {
				ExpiredDAO dao = new ExpiredDAO();
//				System.out.println("Purchase => " + d);
				 
//				  List<Expired> e=new ArrayList<>();
				  return dao.saveExpired();
			}
			
		//  update expired list
			@PutMapping("/update-expired")
			public void updateExpiredList(@RequestBody List<Expired> e) {
				ExpiredDAO dao = new ExpiredDAO();
				System.out.println("Expired update => " + e);
				dao.updateExpiredStatus(e);
			}
...
//			System.out.println("Purchase => "+s);
	...
	@GetMapping("/get-transaction/{startDate}/{endDate}")
	public List<Transaction> getTransaction(@PathVariable Date startDate,@PathVariable Date endDate) {
		TransactionDAO dao = new TransactionDAO();
		List<Transaction> t=new ArrayList<>();
//		Accounts acc=dao.getDetails();
		t=dao.getTransactionReport(startDate, endDate);
